jQuery('document').ready(function($) {
    ...
    var myElem = document.getElementById('form-preview');
    if (myElem == null)
    {
        lengthOfForm = 0;
    }
    else {
        if ($('#form-preview').html().length != null)
        {
            lengthOfForm = $('#form-preview').html().length;
        }
    }
//    if (typeof $('#form-preview').html() != 'undefined' && $('#form-preview').html().length > 0) {
    if (typeof $('#form-preview').html() != 'undefined' && lengthOfForm > 0) {
        previewReady($)
    }
    ...
    $("#wpseo_meta").hide();
    $("#icon-wwm_plugins").remove();
    $(".label-edit > div").css('display', 'block');
    $("#form-preview-wrapper .single-element-edit .button-holder").css('display', 'block');
    $("#form-preview-wrapper .element-label-edit").css('display', 'none');
...
//        $.post(ajaxurl, $('#awesome-survey :input').serializeArray(), function(data) {
        $.post(ajaxurl, $('#post :input').serializeArray(), function(data) {
...
//            count = Number(key) + 1
//            html += '<label for="options-answer-' + key + '">Answer ' + (Number(key) + 1) + '</label><input id="options-answer-' + key + '" type="text" name="options[label][' + key + ']" value="" required="required"> <select required id="map_select_' + key + '" style="float:left;margin-left:30px;margin-top:6px;" name=map_ans[map][' + key + '] onchange="assignVal(' + key + ')"><option value="0">V</options><option value="0">V</options><option value="1">K</options><option value="2">A</options></select>'
...
function generateDynamicDialog(obj) {
    html = '<div class="dyn-diag"><form class="pure-form pure-form-stacked form-horizontal" method="post" action=""><input type="text" name="options[name]" value="'
    html += '" required="required"><label style="display:none;" for="required-checkbox">Required? </label><input style="display:none;" id="required-checkbox" type="checkbox" name="options[validation][required]" value="1"'
    if (typeof obj.validation != 'undefined' && typeof obj.validation.required != 'undefined' && 1 == obj.validation.required) {
        html += ' checked="checked"'
    }
    html += '>'
    if ('undefined' != typeof obj.label) {
        display = ('undefined' != typeof obj.atts && 'undefined' != typeof obj.atts.can_add_options && 'yes' == obj.atts.can_add_options) ? 'block' : 'none'
        html += '<div style="display:none;" class="slider-wrapper"><div style="display:none;" id="edit-slider"></div><div class="slider-legend"></div></div>'
        html += '<p>answers:</p>'
        html += '<div id="edit-answers-holder">'
        for (key in obj.label) {
            count = Number(key) + 1
            html += '<label for="options-answer-' + key + '">Answer ' + (Number(key) + 1) + '</label><input id="options-answer-' + key + '" type="text" name="options[label][' + key + ']" value="" required="required"> <select style="display:none;" required id="map_select_' + key + '" style="float:left;margin-left:30px;margin-top:6px;" name=map_ans[map][' + key + '] onchange="assignVal(' + key + ')"><option value="0">V</options><option value="0">V</options><option value="1">K</options><option value="2">A</options></select>'
            html += '<label style="display:none;" for="options-default-' + key + '">default?<br></label><input style="display:none;"  id="options-default-' + key + '" type="radio" name="options[default]" value="' + key + '"'
            if (key == obj['default']) {
                html += ' checked="checked"'
            }
            html += '>'
...
function assignVal(iteration_num)
{
    var drop_down_map_val = jQuery("#map_select_" + iteration_num).val();
    jQuery("#hidden_" + iteration_num).val(drop_down_map_val);
