...
class Awesome_Surveys_Ajax extends Awesome_Surveys
{
    ...
    private function element_info_inputs($form_element = 'Element_Textbox')
    {
        ...
        $html .= '<label>' . __('The question you are asking:', 'quiz-plugin') . '<br><input type="text" name="options[name]" required></label>';
        if (!empty($validation_elements))
        {
            $html .= '<div class="ui-widget-content field-validation validation ui-corner-all"><h5>' . __('Field Validation Options', 'quiz-plugin') . '</h5>';
            foreach ($validation_elements as $element)
            {
                ...
                {
                    $html .= '<label>' . $element['label_text'] . '<br><' . $element['tag'] . ' ' . ' type="' . $element['type'] . '"  value="' . $element['value'] . '" name="options[validation][' . $element['name'] . ']" checked="checked" ' . $atts . '></label>';
                }
                $rule_count = 0;
                if (!empty($element['data']) && is_array($element['data']))
                {
                    $html .= '<span class="label">' . __('Advanced Validation Rules:', 'quiz-plugin') . '</span>';
            ...
            {
                $html .= '<label>' . __('Display number of characters remaining?', 'quiz-plugin') . '<br><input id="add-countdown" type="checkbox" name="options[add_countdown]"></label>';
            }
            $html .= '</div>';
        }
        ...
        if (in_array($form_element, $needs_options))
        {
            ...
            $html .= '<span class="label">' . __('Number of answers required?', 'quiz-plugin') . '</span><div class="slider-wrapper"><div id="slider"></div><div class="slider-legend"></div></div><div id="options-holder">';
            $html .= $this->options_fields(array('num_options' => 1, 'ajax'        => false));
            $html .= '</div>';
        }

        $html .= '<p><button onclick="validate_first()" class="button-primary">' . __('Add Question', 'quiz-plugin') . '</button></p>';
        $html .= '</div>';
        return $html;
    }
    ...
    public function options_fields($args = array())
    {
        ...
        for ($iterations = 0; $iterations < absint($args['num_options']); $iterations++)
        {
            $label = $iterations + 1;
//            QQQ commented
//            $html .= '<label>' . __('Answer', 'quiz-plugin') . ' ' . $label . '<br><div style="float:left;"><input type="text" name="options[label][' . $iterations . ']" required></div><select required id="map_select_' . $iterations . '" style="float:left;margin-left:30px;margin-top:6px;" name=map_ans[map][' . $iterations . '] onchange="assignVal(' . $iterations . ')"><option value="0">V</options><option value="1">K</options><option value="2">A</options></select><input id="hidden_' . $iterations . '" type="hidden" name="options[value][' . $iterations . ']" value="0"><label>' . __('default?', 'quiz-plugin') . '<br><input type="radio" name="options[default]" value="' . $iterations . '"></label>';
            $html .= '<label>' . __('Answer', 'quiz-plugin') . ' ' . $label . '<br><div style="float:left;"><input type="text" name="options[label][' . $iterations . ']" required></div><select required id="map_select_' . $iterations . '" style="float:left;margin-left:30px;margin-top:6px;" name=map_ans[map][' . $iterations . '] onchange="assignVal(' . $iterations . ')"><option value="0">V</options><option value="1">K</options><option value="2">A</options></select><input id="hidden_' . $iterations . '" type="hidden" name="options[value][' . $iterations . ']" value="0">';
        }
        return $html;
    }
    ...
    public function wwm_survey_validation_elements($elements = array(), $type = '')
    {
        ...
        $elements[]      = array(
            'label_text' => __('required?', 'quiz-plugin'),
            'tag'        => 'input',
            'type'       => 'checkbox',
            'value'      => 1,
            'name'       => 'required',
    ...
    public function get_validation_elements_number($elements)
    {
        ...
        $min = array(
            'label_text' => __('Min number allowed', 'quiz-plugin'),
            'tag'        => 'input',
            'type'       => 'number',
            'name'       => 'min',
        );
        $max = array(
            'label_text' => __('Max number allowed', 'quiz-plugin'),
            'tag'        => 'input',
            'type'       => 'number',
            'name'       => 'max',
        );
    ...
    public function get_validation_elements_text($elements)
    {
        ...
        $maxlength_element  = array(
            'label_text' => __('Maximum Length (in number of characters)', 'quiz-plugin'),
            'tag'        => 'input',
            'type'       => 'number',
            'name'       => 'maxlength',
        );
    ...
    public function generate_preview()
    {
        ...
        $form->addElement(new Element_Button(__('Reset', 'quiz-plugin'), 'button', array('class' => 'button-secondary reset-button', 'name'  => 'reset')));
        ...
        $replacement             = '';
        $save_form               = preg_replace($pattern, $replacement, $save_form);
        $post_content            = str_replace("display:none;", " ", $post_content);
        $preview_form            = str_replace("display:none;", " ", $preview_form);
        $save_form               = str_replace("display:none;", " ", $save_form);
        $data                    = array(array($preview_form . $save_form), array(esc_html($post_content)), array(json_encode($this->existing_elements)));
        // QQQ

        $post_id    = isset($_POST['survey_id']) ? $_POST['survey_id'] : '';
...
//			wp_send_json_error( array( sprintf( '%s %s of %s', __( 'There was an error on line ', 'quiz-plugin' ), __LINE__, __FILE__ ) ) );
//		}
        $post_content            = $this->awesome_surveys_render_form($form_args);
    ...
    public function process_response()
    {
        ...
        if (is_numeric($_POST['survey_id2']))
        {
            $survey_id = absint($_POST['survey_id2']);
        }
        else
        {
...
//        {
//            $data = array(__('Answers not saved. Survey in draft status.', 'quiz-plugin'));
...
//        }
        ...
        {
            $data = array(apply_filters('wwm_survey_no_auth_message', sprintf('<p>%s</p>', __('Your response to this quiz has already been recorded. Thank you!', 'quiz-plugin'))));
            wp_send_json_error($data);
        }
        ...
        {
            $type = $question['type'];
            if ('checkbox' === $type && isset($_POST['question'][$key]))
            {//the answers are an array
                $radio_answers = array();
        ...
        if (!empty($responses))
        {
            ...
            if (is_user_logged_in())
            {
                ...
                add_post_meta($survey_id, "QQ_user_login_" . $user_logged_in_id, $responses, false);
                add_post_meta($survey_id, "QQ_logged_in_" . $user_logged_in_id, $user_logged_in_id, false);
            }
            else
            {
                $random_str                = $this->RandomStringGenerator();
                $_SESSION['response_user'] = "Anonymous_" . $random_str;
                add_post_meta($survey_id, "Anonymous_" . $random_str, $responses, false);
                add_post_meta($survey_id, "Anonymous", $responses, false);
            }
...
//                $table_name = "user_answer_quiz";
//                $user = wp_get_current_user();
//                $user_id = $user->ID;
//                global $wpdb;
//                $wpdb->insert( $table_name, array( 'survey_id' => $_POST['survey_id'], 'user_id' => $user_id ));
