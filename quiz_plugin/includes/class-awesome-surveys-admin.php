...
class Awesome_Surveys_Admin extends Awesome_Surveys
{
    ...
    {
        parent::__construct();
        $this->page_title     = __('Awesome Quiz Options', 'quiz-plugin');
        $this->menu_title     = __('Quiz Options', 'quiz-plugin');
        $this->menu_slug      = 'awesome-surveys.php';
        $this->menu_link_text = __('Quiz', 'quiz-plugin');
    ...
    public function save_post($post_id, $post)
    {
        ...
        if (isset($_POST['existing_elements']))
        {
            ...
            if (json_decode(stripslashes($existing_elements)))
            {
                ...
                $meta_value = isset($_POST['quiz_details']) ? $_POST['quiz_details'] : '';
                if (count($meta_exist) > 0)
                {
...
//		$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
    ...
    public function admin_menu()
    {
        ...
        {
//            $_wwm_plugins_page[0] = add_menu_page('Quiz Plugin', 'Quiz Plugin', 'edit_others_posts', 'wwm_plugins', array(&$this, 'plugin_options'), WWM_AWESOME_SURVEYS_URL . '/images/pl.jpg', '90.314');
            $_wwm_plugins_page[0] = add_menu_page("Quiz Options", 'Quiz Plugin', 'edit_others_posts', 'wwm_plugins', array(&$this, 'plugin_options'), WWM_AWESOME_SURVEYS_URL . '/images/pl.jpg', '90.314');
//			$_wwm_plugins_page[0] = add_menu_page( 'Quiz Plugin PL', 'Quiz Plugin PC', 'edit_others_posts', 'wwm_plugins', array( $this, 'wwm_plugin_links' ), WWM_AWESOME_SURVEYS_URL . '/images/pl.jpg', '90.314' );
            $_wwm_plugins_page[1] = $plugin_panel_version;
        }
        add_submenu_page('wwm_plugins', '', __('My Quizes', 'quiz-plugin'), 'edit_others_posts', 'edit.php?post_type=quiz-plugin');
        add_submenu_page('wwm_plugins', '', __('New Quiz', 'quiz-plugin'), 'edit_others_posts', 'post-new.php?post_type=quiz-plugin');
//        $this->page_hook = add_submenu_page('wwm_plugins', $this->page_title, $this->menu_title, 'edit_others_posts', $this->menu_slug, array(&$this, 'plugin_options'));
    ...
    public function wwm_plugin_links()
    {
        ...
        if (!empty($wwm_plugin_links))
        {
            ...
            echo '<h2>Quiz Plugin</h2>' . "\n";
            echo '<ul>' . "\n";
            foreach ($wwm_plugin_links as $link)
            {

                echo '<li>Quiz Plugin</li>' . "\n";
            }
            echo '</ul>';
        }
    ...
    {
        include_once 'awesome-surveys.php';
        include_once( WWM_AWESOME_SURVEYS_PATH . '/options.php' );

//        add_meta_box('awesome-surveys-options', __('Quiz Options', 'quiz-plugin'), array($this, 'surveys_options'), $this->page_hook, 'normal', 'core');
        add_meta_box('awesome-surveys-email-options', __('Email Options', 'quiz-plugin'), array($this, 'email_options'), $this->page_hook, 'normal', 'core');
    ...
    public function post_row_actions($actions, $post)
    {
        ...
        if ('quiz-plugin' === $post->post_type)
        {
            ...
            if (!empty($show_link))
            {
                $actions['results'] = '<a href="' . $edit_post_link . '&amp;view=results' . '" title="' . __('View Survey Results', 'quiz-plugin') . '">' . __('Results', 'quiz-plugin') . '</a>';
            }
            $actions['duplicate'] = '<a href="' . $duplicate_url . '" title="' . __('Create a copy of this survey', 'quiz-plugin') . '">' . __('Duplicate', 'quiz-plugin') . '</a>';
        }
        return $actions;
    }
    ...
    {
        $screen = get_current_screen();
        if (strpos($screen->id, 'quiz-plugin') > 0 || 'toplevel_page_wwm_plugins' == $screen->id)
        {
            ...
            {
                echo '<div class="updated"><p>' . __('Surveys updated', 'quiz-plugin') . '</p></div>';
                return;
            }
    ...
    public function admin_init()
    {
        ...
        if (isset($_POST['wwm_do_db_upgrade']) && wp_verify_nonce($_POST['wwm_as_db_upgrade'], 'wwm-as-database-upgrade'))
        {
            ...
            ob_start();
            $url = admin_url('edit.php?post_type=quiz-plugin&database_upgraded=true');
            wp_redirect($url);
            exit;
        }
    ...
    public function insert_post_data($data, $postarr)
    {
        ...
        if ('quiz-plugin' == $data['post_type'] && isset($data['post_content']))
        {
    ...
    private function duplicate_survey($post_id)
    {
        ...
        $new_post_data = array(
            ...
            'post_type'    => 'quiz-plugin',
            'post_status'  => 'draft',
            'post_title'   => $post->post_title . ' (' . __('Copy', 'quiz-plugin') . ')',
        );
    ...
    public function survey_editor()
    {
        if (isset($_GET['view']) && 'results' === $_GET['view'])
        {
            ...
            remove_post_type_support('quiz-plugin', 'title');
            remove_meta_box('submitdiv', 'quiz-plugin', 'side');
            add_meta_box('survey-results', __('Survey Results For:', 'quiz-plugin') . ' ' . get_the_title($post_id), array($this, 'survey_results'), 'quiz-plugin', 'normal', 'core');
...
//					$number = ( isset( $_GET['offset'] ) ) ? absint( $_GET['offset'] ) : 0;
                ...
                add_meta_box('respondent-' . $respondent_key, $meta_box_title, array($this, 'answers_by_respondent'), 'quiz-plugin', 'normal', 'core', array($results, $elements, $respondent_key));
            }
        }
        else
        {
//            add_meta_box('quiz_details', __('Quiz Description', 'quiz-plugin'), array($this, 'survey_question_detail'), 'quiz-plugin', 'normal', 'core');
            add_meta_box('quiz_details', ('Quiz Description'), array($this, 'survey_question_detail'), 'quiz-plugin', 'normal', 'core');
            add_meta_box('create_survey', __('Create Quiz', 'quiz-plugin'), array($this, 'survey_builder'), 'quiz-plugin', 'normal', 'core');
            add_meta_box('general-survey-options-metabox', __('General Quiz Options', 'quiz-plugin'), array($this, 'general_survey_options'), 'quiz-plugin', 'normal', 'core');
        }
    }
    ...
    public function answers_by_respondent($post, $args = array())
    {
        ...
        foreach ($questions as $key => $question)
        {
            ...
            if (in_array($question['type'], $has_options))
            {
                ...
                {
                    $response = '<ul class="answers">' . __('Answers', 'quiz-plugin') . "\n";
                ...
                {
                    $response = ( isset($answers[$key]) && isset($question['label'][$answers[$key]]) ) ? '<span class="answer">' . __('Answer', 'quiz-plugin') . ': ' . $question['label'][$answers[$key]] . '</span>' : null;
                }
            }
            else
            {
                $response = ( isset($answers[$key]) && !empty($answers[$key]) ) ? '<span class="answer">' . __('Answer', 'quiz-plugin') . ': ' . $answers[$key] . '</span>' : null;
            }
            $response = (!is_null($response) ) ? $response : '<span class="answer italics">' . __('No response given', 'quiz-plugin') . '</span>';
...
//			echo '<p><span class="italics">' . __( 'Question', 'awesome-surveys' ). ': ' . $label . '</span><br>' . $response . "</p>\n";
    ...
    public function survey_question_detail()
    {
        include_once( 'views/html-survey-question-detail.php' );
    }
    ...
    private function get_options()
    {
        return array(
            ...
            'email_options'   => array(
                ...
                'email_subject'            => __('Thank you for your response', 'quiz-plugin'),
                'mail_to'                  => get_option('admin_email', ''),
                'respondent_email_message' => __('Thank you for your response to a quiz', 'quiz-plugin'),
            )
        );
    }
