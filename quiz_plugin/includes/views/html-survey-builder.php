...
<div id="awesome-survey">
        <!--<div class="overlay"><span class="preloader"><?php // _e( 'JavaScript Loading...', 'quiz-plugin' );  ?></span></div>-->
    ...
    if (empty($has_responses) || 1) :
        ?>
        <div id="survey-elements-buttons">
            <h4><?php _e('Add a form element to your quiz by clicking a button', 'quiz-plugin'); ?></h4>
            <?php
            foreach ($awesome_surveys->buttons as $name => $value)
            {
                if ($name == 'dropdown' || $name == 'radio' || $name == 'checkbox')
                {
        ...
        <div id="current-element-wrapper">
            <hr>
            <h4><?php _e('Configure a quiz question', 'quiz-plugin'); ?></h4>
        <?php
//        global $post;
//        $post_id = $post->ID;
...
//        wp_editor($desc!='' ? $desc : "Enter quiz description here", "quiz_details"); ?>
    ...
    <?php else : ?>
        <h3><?php // _e( 'This survey has responses. Questions can not be edited', 'quiz-plugin' );  ?></h3>
<?php endif; ?>
    <div id="form-preview-wrapper">
        <hr>
        <h4><?php _e('Quiz Preview', 'quiz-plugin'); ?></h4>
        <p>
            <?php
            _e('You can insert this quiz with shortcode: ', 'quiz-plugin');
