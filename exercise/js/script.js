...
jQuery(window).ready(function () {
    ...
    if (jQuery('.a_tab_menu').length > 0) {
        var _selected = getSelected();
        selected = _selected.join('|');
        loadDiagram(jQuery('.a_tab_menu'), selected);
    }
    jQuery('body').on('click', function (event) {
        ...
        if ((!target.is('.m-content-detail') && target.parents('.m-content-detail').length == 0) || target.is('.close')) {
            ...
            parent.find('form.form-two1').slideToggle(400);
            // hide_alt_block and alt_block changed to hide_alt_block1 and alt_block1
            parent.find('div.hide_alt_block1').removeClass('hide');
            parent.find('div.alt_block1').addClass('hide');
        } else if (target.is('a.hidealt')) {
            var parent = target.parents('div.block:first');
            parent.find('form.form-two1').slideToggle(400);
            // hide_alt_block and alt_block changed to hide_alt_block1 and alt_block1
            parent.find('div.hide_alt_block1').addClass('hide');
            parent.find('div.alt_block1').removeClass('hide');
        } else if (target.is('input.alt_1') || target.is('input.alt_2')) {
            ...
            var is_correct = false;
            var parent = target.parents('div.block:first');
            var data_exercise_id = parent.find('input[type="radio"][name="alt"]:checked').attr('data-exercise-id');
            parent.find('.myrow').removeClass('a-right').removeClass('a-wrong');
            if (!parent.find('input[type="radio"][name="alt"]').is(':checked')) {
                alert("Please select any option");
                return;
            ...
            parent.find('a.ex-reload1').removeClass("hide");
            saveAttempt(parent, is_correct);
        }else if(target.is('a.not-deliever')){
            console.log('here');
            var parent = target.parents('div.block:first');
            console.log(parent);
            var data_exercise_id = parent.find('input[type="radio"][name="alt"]:checked').attr('data-exercise-id');
            console.log(data_exercise_id);
            if (!parent.find('input[type="radio"][name="alt"]').is(':checked')) {
                alert("Please select any option");
                return;
            }
            parent.find('p.messages.wrong_' + data_exercise_id).removeClass('hide');
        ...
        } else if (target.is('a.ex-reload1')) {
            var parent = target.parents('div.block:first');
            var selected = parent.find('input[type="radio"][name="alt"]:checked').val();
            var data_exercise_id = parent.find('input[type="radio"][name="alt"]:checked').attr('data-exercise-id');
            var correct = parent.find('input#correct').val();
            parent.find('p.messages.right_' + correct).addClass('hide');
            $("input[type='radio'][data-exercise-id='" + data_exercise_id + "'][name='alt']:checked").parent('div.myrow').removeClass('a-right');
            parent.find('p.messages.wrong_' + selected).addClass('hide');
            $("input[type='radio'][data-exercise-id='" + data_exercise_id + "'][name='alt']:checked").parent('div.myrow').removeClass('a-wrong');
            $("input[type='radio'][data-exercise-id='" + data_exercise_id + "'][name='alt']:checked").removeAttr("checked");
            parent.find('a.ex-reload1').addClass("hide");
        }
    });
});
jQuery(document).ready(function () {
    jQuery('body').on('click', '.a_tab_menu li a.year', function () {
    ...
    jQuery(document).ready(function () {
        jQuery('a[data-tab-id="tab2"]').attr('href', 'javascript:void(0)');
    });
    jQuery('.a-rating-section').on('click', '.d-block-opener', function () {
        ...
        element.find('li.active').each(function (index) {
            ...
            if(typeof(_terms[year]) == 'undefined') {
                _terms[year] = {};
            }
            if(term == 'spring'){
                _terms[year].spring = 1;
            }
            else if(term == 'autumn'){
                _terms[year].autumn = 1;
            }
        });
        var terms = [];
        $index = 0;
        for(index in _terms){
            terms[$index] = {year: index, spring: _terms[index].spring, autumn: _terms[index].autumn};
            $index++;
        }
    ...
    jQuery('body').on('change', '.checkbox-extype', function () {
        var self = this;
        if(jQuery(self).is(':checked') && !jQuery('.checkbox-tools').is(':checked')){
            jQuery('.checkbox-tools').attr('checked', 'checked');
        }
        else if(!jQuery('.checkbox-extype').is(':checked')){
            jQuery('.checkbox-tools').removeAttr('checked');
        }
        var _selected = getSelected();
        selected = _selected.join('|');
        loadDiagram(jQuery('.a_tab_menu'), selected);
    });
    jQuery('body').on('change', '.checkbox-tools', function () {
        if(!jQuery('.checkbox-tools').is(':checked')){
            jQuery('.checkbox-extype').removeAttr('checked');
        }
        else if(!jQuery('.checkbox-extype').is(':checked')){
            jQuery('.checkbox-extype').attr('checked', 'checked');
        }
        var _selected = getSelected();
        selected = _selected.join('|');
        loadDiagram(jQuery('.a_tab_menu'), selected);
    });
    jQuery("#user_id").on("keypress", function (event) {
        var fieldVal = jQuery("#user_id").val();
        if (fieldVal != '' && event.which == 13) {
            jQuery('#users_search').submit();
    ...
    jQuery(window).on('load', function () {
        var fieldVal = jQuery("#user_id").val();
        if (fieldVal != "") {
            jQuery('.cross_search').removeClass('hide_img');
        }
    });
    setTimeout(function () {
        jQuery('.tab5_view').on('click', function () {
            jQuery('a[data-tab-id="tab2"]').removeClass('pws_tab_active');
            jQuery('div[data-pws-tab="tab5"]').removeClass('pws_hide');
            jQuery('div[data-pws-tab="tab5"]').css({'display': "block"});
            jQuery('html,body').animate({scrollTop: jQuery('div[data-pws-tab="tab5"]').offset().top - 1500}, 'slower');
            jQuery('div[data-pws-tab="tab2"]').css({'display': "none"});
        });
    }, 500);
});
...
function getSelected() {
    var _selected = [];
    jQuery('ul.a_radio_links li').each(function (index) {
        if(jQuery(this).find('input[type="checkbox"]:first').is(':checked')){
            _selected.push(jQuery(this).find('input[type="checkbox"]:first').data('element'));
        }
    });
    return _selected;
}
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            if (type == 'loadDiagram') {
                ...
                jQuery('a.a-term.open').remove();
                jQuery('.a_tab_menu li.active').append(response.terms);
                ...
                // Start resizing all images
                $('div.m-content-detail img').each(function () {
                    $(this).load(function () {
                        var src = $(this).attr('src');
                        if(src.indexOf('s3-us-west-2.amazonaws.com') != -1) {
                            var the_width = $(this).prop('naturalWidth');
                            if (the_width == 0) {
                                the_width = $(this).width();
                            }
                            if (the_width > 400) {
                                var new_width = Math.round(the_width / 2);
                                if (new_width > 700) {
                                    var new_width = Math.round(the_width / 3);
