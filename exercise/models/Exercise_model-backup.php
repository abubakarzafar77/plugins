...
class Exercise_model{
    ...
    public function get_category_solution($__where, $course, $terms, $term_id){
        ...
        foreach($terms as $term) {
            ...
            $sql = "SELECT we.id FROM " . $this->table . " we INNER JOIN $this->course_table wec ON we.id=wec.exercise_id WHERE 1=1" . $where.$_where.$__where.($this->publish?' AND '.$this->publish:'')." AND sub_chapter_id='".$term_id."'";
            $result = $this->db->get_results($sql, ARRAY_A);
    ...
    public function get_exercise_userdata($exercise_id){
        $sql = "SELECT e.exercise_name as exercise_name_detail, ea.exercise_id, ea.user_id, ea.created_at, e.corr_alternative as correct_alt, ea.alt_choose as answered_alt FROM $this->attempts_table ea JOIN $this->table e ON ea.exercise_id=e.id WHERE ea.exercise_id=$exercise_id ORDER BY created_at DESC";
        return $this->db->get_results($sql, ARRAY_A);
    }
    ...
    public function get_exercise_solution_date($exercise_id){
        $user_id = get_current_user_id();
        $sql = "SELECT created_at FROM $this->attempts_table WHERE correct=1 AND exercise_id=$exercise_id AND user_id=$user_id LIMIT 1";
