...
?>
    <?php if(count($my_jobs) > 1){?>
        <div class="w-clearfix">
            <div class="j-pagging">
                <a href="javascript:void(0);" class="previous_job" data-id="<?php echo $count-1;?>">< Previous job</a>
                <a href="javascript:void(0);" class="next_job" data-id="<?php echo $count+1;?>">Next job ></a>
            </div>
        </div>
    <?php }?>
            ...
            <div class="w-clearfix j-outer job_outer" id="job_<?php echo $count;?>"<?php echo $style;?>>
                <section class="job w-clearfix">
                    <div class="j-top-meta w-clearfix">
                        <div class="j-time"><strong>Time: <?php echo date('d.m.Y   H:i', strtotime($job->webinar_date_time));?></strong></div>
                        ...
                        <div class="j-delete-btn"><a href="<?php echo wp_nonce_url(home_url('studiekamerat?page=my_jobs&action=delete_job&job_id='.$job->ID), 'delete_job', 'delete_job');?>" onclick="return confirm('Are you sure you want to delete?');" class="btn j-btn">Delete job</a></div>
                    </div>
                    ...
                    <?php foreach($offers as $offer):?>
                        ...
                        <?php }else{?>
                            <div class="j-full-profile w-clearfix">
                                <div class="j-profile">
                                    <img src="<?php echo $model->get_teacher_avatar($offer->offer_teacher_id);?>" style="width: 150px;" />
                                    ...
                                    <div><strong><?php echo get_user_meta($offer->offer_teacher_id, 'first_name', true);?> <?php echo get_user_meta($offer->offer_teacher_id, 'last_name', true);?></strong></div>

                                    <div class="j-see-full-profile">Om meg video<br /><a href="<?php echo home_url('studiekamerat?page=teacher&id=' . $offer->offer_teacher_id);?>" target="_blank">Se full profil</a></div>
                                </div>
                                <div class="j-description">
                                ...
                                <div class="j-choose-btn">
                                    <?php if(!$offer->is_accepted && !$paid){?>
                                        <a href="<?php echo home_url('studiekamerat?page=pay&offer='.$offer->ID.'&job='.$job->ID);?>" id="<?php echo 'btn_'.$offer->ID.'_'.$job->ID;?>" class="btn j-btn fancybox.ajax fancybox">Choose <br />and pre pay</a>
