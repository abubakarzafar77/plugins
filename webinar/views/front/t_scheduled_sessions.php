<?php if(isset($_REQUEST['scheduled'])){?>
    <div class="col-md-12">
        <div class="alert alert-success" role="alert">Webinar is setup successfully.</div>
    </div>
<?php }?>
<?php
$count = 0;
$style = ' style="display: none;"';
if(count($scheduled_sessions) > 0){
    ?>
    <?php if(count($scheduled_sessions) > 1){?>
        <div class="w-clearfix">
            <div class="j-pagging">
                <a href="javascript:void(0);" class="previous_job" data-id="<?php echo $count-1;?>"><</a>
                <span class="shown">1</span><span>/</span><span class="of"><?php echo count($scheduled_sessions);?></span>
                <a href="javascript:void(0);" class="next_job" data-id="<?php echo $count+1;?>"> ></a>
            </div>
        </div>
    <?php }?>
    <div class=" my_jobs">
        <?php foreach($scheduled_sessions as $job):?>
            <?php
            ...
            <div class="w-clearfix j-outer job_outer" id="job_<?php echo $count;?>"<?php echo $style;?>>
                <section class="job w-clearfix">
                    <div class="j-top-meta w-clearfix">
                        ...
                        <div class="j-duration">Varighet:<?php echo str_replace("-", " ", $job->webinar_duration);?></div>
                        <div class="j-max-price">
                            <p>Pris:<?php echo ucfirst(str_replace("-", " ", $job->webinar_budget));?></p></div>
                        <div class="j-delete-btn">
                            <a href="<?php echo home_url('studiekamerat?page=setup&job_id='.$job->ID);?>" class="btn j-btn fancybox.ajax fancybox">Innstillinger</a>
                            <a href="<?php echo $job->webinar_url;?>" class="btn j-btn" target="_blank">Grupperom</a>
                        </div>
                    </div>
