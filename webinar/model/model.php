...
class Model{
    ...
    private $db;
    private $config;
    private $jobs_table,$categories_table,$hearts_table,$offers_table,$ratings_table,$groups_table;
    private $env = 'sandbox';
    private $merchent_id = '';
    private $public_key = '';
    private $private_key = '';
    ...
    function Model(){
        ...
        $this->groups_table = str_replace('[WPDBPREFIX]', $wpdb->prefix, $this->config->plugin_job_groups_table);
    ...
    public function save_webinar_request($post, $without_nonce=false){
        if($without_nonce){
            $data = $post;
    ...
    public function save_exercise_group($data){
        $_data = array(
            'packages_deal'=>$data['packages_deal'],
            'teacher_id'=>get_current_user_id(),
            'send_to'=>$data['email_address'],
            'created_at'=>time()
        );
        if($this->db->insert($this->groups_table, $_data)) {
            $group_id = $this->db->insert_id;
            $user = WP_User::get_data_by('email', $data['email_address']);
            $budget = $this->config->teacher_budget->$data['packages_deal'];
            foreach($data['_date_time'] as $key=>$value) {
                $post = array(
                    'webinar_date_time' => $value,
                    'webinar_duration' => (($data['_duration'][$key] == .5) ? '30-minutter' : $data['_duration'][$key] . '-timer'),
                    'webinar_description' => '',
                    'webinar_education_level' => '',
                    'webinar_teacher' => get_current_user_id(),
                    'webinar_budget' => $budget,
                    'webinar_user_id' => $user->ID,
                    'webinar_files' => '',
                    'webinar_status' => 'posted',
                    'webinar_group_id'=> $group_id
                );
                $this->save_webinar_request($post, true);
            }
            return $group_id;
        }else
            return false;
    ...
    public function count_teacher_group_webinars(){
        return $this->db->get_var("SELECT COUNT(*) AS CNT FROM ".$this->groups_table." WHERE teacher_id= '".get_current_user_id()."'");
    }

    public function get_teacher_groups(){
        return $this->db->get_results($this->db->prepare("SELECT * FROM $this->groups_table WHERE teacher_id=%d ORDER BY id DESC", get_current_user_id()));
    }

    public function get_teacher_group_time($group_id){
        return $this->db->get_results( 'SELECT * FROM '.$this->jobs_table.' WHERE webinar_group_id='.$group_id, ARRAY_A );
    }

    public function get_job_by_group($group_id){
        return $this->db->get_results(
            $this->db->prepare(
                "SELECT * FROM $this->jobs_table WHERE webinar_group_id=%d ORDER BY id DESC",
                $group_id
            )
        );
    }
