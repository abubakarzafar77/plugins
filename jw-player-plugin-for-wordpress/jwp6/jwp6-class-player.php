...
class JWP6_Player {
    ...
    */
    ...
    public function embedcode($id, $file = null, $playlist=null, $image = null, $config = null, $streamer = null) {
        if ( ! is_null($config) ) {
        ...
        if ( $file && ! $playlist ) {
            if ( $this->get('streamer') ) {
				$file = $this->get('streamer') .'/mp4:'. $file;
				//Commented by Muhammad Saleem
                //$file = $this->get('streamer') . $file;
            }elseif($streamer){
				$file = $streamer .'mp4:'. $file;
			}
			//echo $file;exit;
            if ( "/" == substr($file, 0, 1) ) {
		...
		if($browser->getBrowser() == Browser::BROWSER_SAFARI || $browser->getBrowser() == Browser::BROWSER_IE/* && $browser->getPlatform() == Browser::PLATFORM_APPLE && $browser->getVersion() < 7*/ && (isset($params['html5_file']) || $params['primary'] == 'html5')){
			if(!isset($params['html5_file'])){
				$file = explode(':', $params['file']);
				$file = $file[2];
				$html5video = el_s3_getTemporaryLink('AKIAJY3WWVWKYSF46GZA', 'qKeZnit7SQggLttu1M/+JjXm1MLPzj/FgRvQE+T+', 'mattevideo', '/'.$file,15);
				$params['html5_file'] = $html5video;
			}
			$embedcode = '<video width="'.$params['width'].'" height="'.$params['height'].'" controls>';
			$embedcode .= '<source src="'.$params['html5_file'].'" type="video/mp4">';
			$embedcode .= 'Your browser does not support the video tag.';
			$embedcode .= '</video>';		
		}else if(($browser->getPlatform() == Browser::PLATFORM_ANDROID) && (isset($params['html5_file']) || $params['primary'] == 'html5')){
			if(!isset($params['html5_file'])){
				$file = explode(':', $params['file']);
				$file = $file[2];
				$html5video = el_s3_getTemporaryLink('AKIAJY3WWVWKYSF46GZA', 'qKeZnit7SQggLttu1M/+JjXm1MLPzj/FgRvQE+T+', 'mattevideo', '/'.$file,15);
			...
			$embedcode = '<video   controls style="max-height:416px;"> ';
			$embedcode .= '<source src="'.$params['html5_file'].'" type="video/mp4">';
			$embedcode .= 'Your browser does not support the video tag.';
			$embedcode .= '</video>';
			//echo $params['width'].'; height:'.$params['height'];
			
		}
		else if(($browser->getPlatform() == Browser::PLATFORM_IPAD) && (isset($params['html5_file']) || $params['primary'] == 'html5')){
			if(!isset($params['html5_file'])){
				$file = explode(':', $params['file']);
				$file = $file[2];
				$html5video = el_s3_getTemporaryLink('AKIAJY3WWVWKYSF46GZA', 'qKeZnit7SQggLttu1M/+JjXm1MLPzj/FgRvQE+T+', 'mattevideo', '/'.$file,15);
		...
		}else if(($browser->getPlatform() == Browser::PLATFORM_IPHONE ) && (isset($params['html5_file']) || $params['primary'] == 'html5')){
			if(!isset($params['html5_file'])){
				$file = explode(':', $params['file']);
				$file = $file[2];
				$html5video = el_s3_getTemporaryLink('AKIAJY3WWVWKYSF46GZA', 'qKeZnit7SQggLttu1M/+JjXm1MLPzj/FgRvQE+T+', 'mattevideo', '/'.$file,15);
			...
			$embedcode = '<video loop autoplay controls width="'.$params['width'].'" height="'.$params['height'].'" src="'.$params['html5_file'].'" type="video/mp4" ></video>';
			//$embedcode = '<video width="'.$params['width'].'" height="'.$params['height'].'" controls preload="auto" autoplay >';
//			$embedcode .= '<source src="'.$params['html5_file'].'" type="video/mp4;">';
//			$embedcode .= 'Your browser does not support the video tag.';
//			$embedcode .= '</video>';
//			//echo $params['width'].'; height:'.$params['height'];
